...
import { GitProject } from '../../pages/gitlab/gitlab.component';


export type GitlabRefType = 'branches' | 'tags' | 'commits';

@Component({
  selector: 'app-gitlab-selector-dialog',
  imports: [
    CommonModule,
    FormsModule,
    MatInputModule,
  ...
  templateUrl: './gitlab-selector-dialog.component.html',
  styleUrl: './gitlab-selector-dialog.component.scss'
})
export class GitlabSelectorDialogComponent {
  ...
  onSelect(): void {
    ...
    this.apiGitlabService.projectFileDownload(this.data.provider, gitProject.id, { projectId: this.projectId, systemPrompt: this.systemPrompt }, refType, refId).subscribe(project => {
      console.log(project);
      ...
      this.threadService.upsertThreadGroup(this.projectId, threadGroup).pipe(
        switchMap(_threadGroup => {
          ...
          return safeForkJoin(newThreadGroup.threadList.map(thread => {
            ...
            const messageGroup = this.messageService.initMessageGroup(thread.id, undefined, 'system', [contentPartPrompt, contentPartFile],);
